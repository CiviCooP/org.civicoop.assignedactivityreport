...
class CRM_Assignedactivityreport_Form_Report_AssignedActivityReport extends CRM_Report_Form {
  ...
  public function where($recordType = NULL) {
    ...
    if ($contactID = $session->get('userID')) {
      $clauses[] = "{$this->_aliases['civicrm_activity_contact']}.activity_id IN
                           (SELECT activity_id FROM civicrm_activity_contact WHERE record_type_id = 1 AND contact_id = {$contactID})";
    }
